...
void tcp_work(int fd){
    ...
    while (1){
        ...
        /// CHATROOM RELATED ///
            ...
            // Check if chatroom exists, if exist, send -1; else, 0
            int rtn{checkRoom(username)};
            if (!rtn){
                // Insert into database
                insertRoom(username, cmd_arg[1]);
                msg.assign("0");
            }
            else 
                msg.assign("-1");
            ...
            int port{checkRoom(cmd_arg[1])};
            int rtn{};
            if (port != 0)
                rtn = checkStatus(cmd_arg[1]);
            if (!rtn)
                port = 0;
            msg.assign(std::to_string(port));
        }

        else if (cmd_arg[0].compare("leave-chatroom") == 0){
            // Get client username and change the chatroom status into 0
            std::string username = retName(std::to_string(key));
            updateRoomStatus("close", username);
            msg.clear();
            cmd_arg.clear();
            continue;
        }
            ...
            // Get client username and change the chatroom status into 1
            std::string username = retName(std::to_string(key));
            updateRoomStatus("open", username);
            msg.clear();
            cmd_arg.clear();
            continue;
        }
...
void dbInitialise(){
    ...
    /// CHATROOM INFO ///
    sql = "DROP TABLE IF EXISTS CHATROOM;"
